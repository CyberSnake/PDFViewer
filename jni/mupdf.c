...
JNI_FN(MuPDFCore_drawPage)(JNIEnv *env, jobject thiz, jobject bitmap,
	...
	LOGE(
			"Rendering page(%d)=%dx%d patch=[%d,%d,%d,%d]", pc->number, pageW, pageH, patchX, patchY, patchW, patchH);
...
JNI_FN(MuPDFCore_updatePageInternal)(JNIEnv *env, jobject thiz, jobject bitmap,
		int page, int pageW, int pageH, int patchX, int patchY, int patchW,
		int patchH) {
	AndroidBitmapInfo info;
	void *pixels;
	int ret;
	...
	fz_try(ctx)
	{
		...
		for (crect = hq ? pc->hq_changed_rects : pc->changed_rects; crect;
			...
			LOGI(
					"Update rectangle (%d, %d, %d, %d)", abox.x0, abox.y0, abox.x1, abox.y1);
...
JNI_FN(MuPDFCore_text)(JNIEnv * env, jobject thiz) {
	...
	fz_try(ctx)
	{
		...
		if (barr == NULL)
			fz_throw(ctx, "NewObjectArray failed");

		for (b = 0; b < text->len; b++) {
			fz_text_block *block = &text->blocks[b];
			jobjectArray *larr = (*env)->NewObjectArray(env, block->len,
					textLineClass, NULL);
			if (larr == NULL)
				fz_throw(ctx, "NewObjectArray failed");
			...
			for (l = 0; l < block->len; l++) {
				...
				jobjectArray *sarr = (*env)->NewObjectArray(env, line->len,
						textSpanClass, NULL);
				if (sarr == NULL)
					fz_throw(ctx, "NewObjectArray failed");
				...
				for (s = 0; s < line->len; s++) {
					...
					for (c = 0; c < span->len; c++) {
						...
						jobject cobj = (*env)->NewObject(env, textCharClass,
								ctor, ch->bbox.x0, ch->bbox.y0, ch->bbox.x1,
								ch->bbox.y1, ch->c);
						if (cobj == NULL)
							fz_throw(ctx, "NewObjectfailed");
...
JNIEXPORT void JNICALL
JNI_FN(MuPDFCore_addStrikeOutAnnotationInternal)(JNIEnv * env, jobject thiz,
		jobjectArray lines) {
	globals *glo = get_globals(env, thiz);
	fz_context *ctx = glo->ctx;
	fz_document *doc = glo->doc;
	...
	fz_try(ctx)
	{
		...
		for (i = 0; i < n; i++) {
			...
			if (!stroke || stroke->linewidth != thickness) {
				if (stroke) {
					// assert(path)
					fz_stroke_path(dev, path, stroke, &ctm, fz_device_rgb,
							color, 1.0);
					LOGI("Path stroked");
...
JNI_FN(MuPDFCore_getWidgetAreasInternal)(JNIEnv * env, jobject thiz,
	...
	count = 0;
	for (widget = fz_first_widget(idoc, pc->page); widget; widget =
			fz_next_widget(idoc, widget))
		count++;
...
JNIEXPORT jobject JNICALL
JNI_FN(MuPDFCore_getBoundingBoxInternal)(JNIEnv * env, jobject thiz, float x, float y, int screenWidth, int screenHeight)
{
	...
	fz_rect afz_infinite_rect = { 1, 1, -1, -1 };
	...
	fz_try(ctx)
	{
		...
		for (block = text->blocks; block < text->blocks + text->len; block++) // for 1
		{
			/*
			 So, users must input the coordinates of 2 points that are the opposites corners of a rectangle and then the app
			 will check if they are the coordinates of opposites corners.
			 If they are, then users must input the coordinates of a point and this app will verify if the
			 point is inside the rectangle or not and will display a message.
			 */
			// a1 and a2 are the coordinates of the corner A of the rectangle, b1 and b2 of the corner B and p1 and p2 are the coordinates of the point
			...
			// I'll check if the coordinates are from the opposites corners of a rectangle in all 4 trigonometric quadrants
			if (a1 < b1 && a2 < b2 || a1 < b1 && a2 > b2
					|| b1 < a1 && a2 < b2 || b1 < a1 && b2 < a2
					|| a1 < b1 && a2 < b2 || a1 < b1 && a2 > b2
					|| b1 < a1 && a2 < b2 || b1 < a1 && b2 < a2) // if 1
			{
				//check if the point is inside the rectangle.
				if (a1 < p1 && p1 < b1 && a2 < p2 && p2 < b2
						|| a1 < p1 && p1 < b1 && b2 < p2 && p2 < a2
						|| b1 < p1 && p1 < a1 && a2 < p2 && p2 < b2
						|| b1 < p1 && p1 < a1 && a2 > p2 && p2 > b2
						|| a1 < p1 && p1 < b1 && a2 < p2 && p2 < b2
						|| a1 < p1 && p1 < b1 && b2 < p2 && p2 < a2
						|| b1 < p1 && p1 < a1 && b2 < p2 && p2 < a2
						|| b1 < p1 && p1 < a1 && a2 < p2 && p2 < b2) // if 2
				{
					rect.x0 = a1;
					rect.y0 = a2;
					rect.x1 = b1;
					...
					if (ctor == NULL)
						continue;
					rectF = (*env)->NewObject(env, rectFClass, ctor, rect.x0, rect.y0, rect.x1, rect.y1);
					break;
				}
			} // if 1 end
		} // for 1 end

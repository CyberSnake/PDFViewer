...
//		int docWidth  = v.getMeasuredWidth();
	...
	protected RectF getContainingBoundingBox(float x, float y, int screenWidth, int screenHeight) {
		return null;
	};
...
//		MuPDFPageView pageView = (MuPDFPageView) mChildViews.get(mCurrent);
//		Log.v("", "pageView.getWidth(): " + pageView.getWidth());
//		Log.v("", "pageView.getLeft(): " + pageView.getLeft());
//		Log.v("", "pageView.mSize.x: " + pageView.mSize.x);
	...
	public boolean onTouchEvent(MotionEvent event) {
		...
		if (event.getActionMasked() == MotionEvent.ACTION_UP) {
			...
			View v = mChildViews.get(mCurrent);
			if (v != null) {
				if (mScroller.isFinished() & !mDoubleTap) {
					// If, at the end of user interaction, there is no
					// current inertial scroll in operation then animate
					// the view onto screen if necessary
					// commented as creating problem in double tap 
					slideViewOntoScreen(v);
				}
...
//		requestLayout();
	...
	public boolean onDoubleTap(MotionEvent arg0) 
	{
		...
		float scale = pageView.mSourceScale*(float)pageView.getWidth()/(float)pageView.mSize.x;
		float docRelX = (arg0.getX() - pageView.getLeft())/scale;
		float docRelY = (arg0.getY() - pageView.getTop())/scale;
		
		float touchX = arg0.getX();
		float touchY = arg0.getY();
		float scaleFactor = 2;	//Default scale factor if rect not found or user tapped on image;
		
		DisplayMetrics metrics = getResources().getDisplayMetrics();
		// Getting the rect containing text
		RectF rect = getContainingBoundingBox(docRelX, docRelY, metrics.widthPixels, metrics.heightPixels);
		
		if(rect != null)
		{
			updateRect(rect, scale);
			
			//Uncomment following line To enable rect visualization
//			pageView.rect = rect;
			
			// Calculating scaling factor
			scaleFactor = 0.95f/Math.max(rect.width()/pageView.mSize.x, rect.height()/pageView.mSize.y);
		}

		//<---------comment starts here for enable rect visualization---------->
		float previousScale = mScale;
		mScale = Math.min(Math.max(scaleFactor, MIN_SCALE), MAX_SCALE);
		
		// Checking if user double tapped on the zoomed page then returning it to default scaling and previous scroll values
		if(previousScale > 1)
		{
			mScale = 1;
			mScaling = false;
			mScrollDisabled = false;
			mDoubleTap = false;
			mXScroll = -pageView.getLeft();
			mYScroll = -pageView.getTop();
			requestLayout();
			return true;
		}
		
		// Calculate the amount to scroll in x direction
		mXScroll = getXScrollValue(rect, pageView, touchX, scaleFactor);
		mYScroll = getYScrollValue(rect, pageView, touchY, scaleFactor);
		
		//<---------comment ends here for enable rect visualization---------->
	...
	private int getYScrollValue(RectF rect, MuPDFPageView pageView, float touchY, float scaleFactor) {

		if(rect != null)
		{
			float scrolledTop = rect.top * mScale;
			mYScroll = (int) (rect.top - scrolledTop + mYScroll);
		}else
		{
			// Work out the focus point relative to the view top left
			int viewFocusY = (int)touchY - pageView.getTop() + mYScroll;
			// Scroll to maintain the focus point
			mYScroll += viewFocusY - viewFocusY * scaleFactor;
		}
		
		return mYScroll;
	}

	private int getXScrollValue(RectF rect, MuPDFPageView pageView, float touchX, float scaleFactor)
	{
		if(rect != null)
		{
			int doc_spaced_X = (int) (rect.left - pageView.getLeft() + mXScroll);
			mXScroll += 10 - doc_spaced_X * mScale;
		}else
		{
			// Work out the focus point relative to the view top left
			int viewFocusX = (int)touchX - pageView.getLeft() + mXScroll;
			
			// Scroll to maintain the focus point
			mXScroll += viewFocusX - viewFocusX * scaleFactor;
		}
	...
	private RectF updateRect(RectF rect, float scaleRatio) {
		
		rect.left *= scaleRatio;
		rect.top *= scaleRatio;
		rect.right *= scaleRatio;
		rect.bottom *= scaleRatio;
		return rect;
	}
